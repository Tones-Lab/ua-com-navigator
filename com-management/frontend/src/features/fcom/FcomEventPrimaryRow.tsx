        ...
        <div>
              ...
              {(() => {
                ...
                return (
                  <>
                    <span
                      className={
                        isFieldHighlighted(eventPanelKey, 'Summary')
                          ? 'label label-warning'
                          : 'label'
                      }
                    >
                    ...
                    {overrideTargets.has('$.event.Summary') && (
                      <div className="override-summary" tabIndex={0} {...overrideTooltipHoverProps}>
                        ...
                        >
                          ...
                          {hasEditPermission &&
                            panelEditState[eventPanelKey] &&
                            overrideValueMap.has('$.event.Summary') && (
                              <button
                                type="button"
                                className="pill-close"
                                aria-label="Remove Summary override"
                                onClick={() =>
                                  openRemoveOverrideModal(obj, 'Summary', eventPanelKey)
                                }
                              >
                                ×
                              </button>
          ...
          {panelEditState[eventPanelKey] ? (
            (() => {
              ...
              const draftValue = panelDrafts?.[eventPanelKey]?.event?.Summary;
              const displayValue = draftValue ?? (editable.display ?? '');
              return (
                <input
                  className={`${
                    isFieldHighlighted(eventPanelKey, 'Summary')
                  ...
                  disabled={
                    ...
                    (isFieldPendingRemoval(eventPanelKey, 'Summary') && isFieldNew(obj, 'Summary'))
                  }
                  title={
                    !editable.editable
              ...
              {(() => {
                ...
                return (
                  <>
                    <span
                      className={
                        isFieldHighlighted(eventPanelKey, 'Severity')
                          ? 'label label-warning'
                          : 'label'
                      }
                    >
                    ...
                    {overrideTargets.has('$.event.Severity') && (
                      <div className="override-summary" tabIndex={0} {...overrideTooltipHoverProps}>
                        ...
                        >
                          ...
                          {hasEditPermission &&
                            panelEditState[eventPanelKey] &&
                            overrideValueMap.has('$.event.Severity') && (
                              <button
                                type="button"
                                className="pill-close"
                                aria-label="Remove Severity override"
                                onClick={() =>
                                  openRemoveOverrideModal(obj, 'Severity', eventPanelKey)
                                }
                              >
                                ×
                              </button>
          ...
          {panelEditState[eventPanelKey] ? (
            (() => {
              ...
              const processorTitle = processorSummary
                ? `Value set by processor • ${processorSummary}`
                : 'Value set by processor';
              const draftValue = panelDrafts?.[eventPanelKey]?.event?.Severity;
              const displayValue =
                draftValue ?? (getEditableValue(getEffectiveEventValue(obj, 'Severity')).display ?? '');
              return (
                <input
                  className={`${
                    isFieldHighlighted(eventPanelKey, 'Severity')
                  ...
                  disabled={
                    isFieldLockedByBuilder(eventPanelKey, 'Severity') ||
                    (isFieldPendingRemoval(eventPanelKey, 'Severity') &&
                      isFieldNew(obj, 'Severity'))
                  }
                  title={
