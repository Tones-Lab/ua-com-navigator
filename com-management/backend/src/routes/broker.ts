...
router.get('/servers', async (req: Request, res: Response) => {
  try {
    ...
    const data = await uaClient.getBrokerServers();
    res.json(data);
  } catch (error: any) {
    logger.error(`Broker servers error: ${error.message}`);
    res.status(500).json({ error: error.message || 'Failed to load broker servers' });
  }
});
